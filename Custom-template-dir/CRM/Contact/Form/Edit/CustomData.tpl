...
{/if}
  ...
  cj(function(){
	...
	cj("tr.custom_14_-1-row").hide();
	cj("input[name='custom_11_-1']").change(
	    function(e)
	    {
		cj("tr.custom_12_-1-row").hide();
		cj("tr.custom_13_-1-row").hide();
		cj("tr.custom_14_-1-row").hide();
		var selectVal = cj(this).val();
		if(selectVal=="Road")
		{
		    cj("tr.custom_12_-1-row").show();
		}
		else if(selectVal=="Water")
		{
		    cj("tr.custom_13_-1-row").show();
		}	
		else if(selectVal=="Air")
		{
		    cj("tr.custom_14_-1-row").show();
		}
	    });
   });
    {/literal}
